...
# В name и lore работают плейсхолдеры прочности (как у голограмм):
#   %durability%      — накопленная прочность (у ядра, вернувшегося при
#                       поломке привата) или стартовая у свежего предмета
#   %max_durability%  — потолок прочности типа
recipe:
...
explosions:                 # каким видам взрывов можно вредить ядру
  ...
  WIND_CHARGE: false        # заряд ветра (игрок и бриз)
```
